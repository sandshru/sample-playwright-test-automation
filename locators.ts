...
export const getCookieConsentDialogLocators = (page: Page) => ({
  cookieConsentDialog: page.getByRole("dialog", {
    name: "Software Testing Board asks",
  }),
...
export const getProductTileLocators = (page: Page) => {
  ...
  return {
    ...
    addToCartButton: (index: number) =>
      productTile.nth(index).locator("button.tocart"),
    addToWishListButton: (index: number) =>
      productTile.nth(index).locator("a.towishlist"),
    addToCompareButton: (index: number) =>
      productTile.nth(index).locator("a.tocompare"),
  };
};